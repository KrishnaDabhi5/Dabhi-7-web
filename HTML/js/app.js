...
$(function() {
  ...
  // Fallback: if for some reason images never finish loading,
  // hide the loader after a short delay so the site is still usable.
  const loaderFallback = setTimeout(() => {
    const loader = document.getElementById("loader");
    const loaderContent = document.getElementById("loaderContent");
    if (loader && loaderContent && !loader.classList.contains("loaded")) {
  ...
  $("#contact-form").submit(function() {
		...
		$.ajax({
			...
			if (response && response.success) {
				...
				alert('Error: ' + (response.error || 'Failed to send message. Please try again.'));
			}
		}).fail(function(xhr, status, error) {
			console.error('AJAX Error:', status, error);
			console.error('Response:', xhr.responseText);
			if (xhr.status === 0) {
				alert('Error: Cannot connect to server. Make sure you are running a PHP server.\n\nFor local testing, run: php -S localhost:8000\nThen open: http://localhost:8000/index-svg.html');
			} else if (xhr.status === 404) {
				alert('Error: PHP file not found. Make sure telegram-send.php is in the same folder as your HTML file.');
			} else {
				alert('Error: Failed to send message. Status: ' + xhr.status + '\nError: ' + error);
			}
		});
		return false;
	});
